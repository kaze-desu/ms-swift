...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(self,
        ...
        # DeepSpeed compatibility: auto-adjust gradient checkpointing to avoid conflicts
        self._configure_gradient_checkpointing_for_deepspeed()
        ...
        if template is None:
            raise ValueError("template is None - cannot initialize processing_class")
        
        self.processing_class = template.tokenizer
        if self.processing_class is None:
            raise ValueError("template.tokenizer is None - cannot initialize processing_class")
        ...
        if self.use_liger_loss:
            ...
            skip_ref_model_for_liger = self.args.enable_sagpo and not self.args.sagpo_use_kl_penalty
            self.liger_grpo_loss = LigerFusedLinearGRPOLoss(
                beta=self.beta,
                epsilon_low=self.epsilon_low,
                epsilon_high=self.epsilon_high,
                temperature=self.temperature,
                use_ref_model=self.beta != 0.0 and not skip_ref_model_for_liger,
        ...
        # More specific flags for different aspects of dynamic sampling
        self.has_variable_rollout_counts = False  # Whether rollout outputs != rollout inputs
        self.has_variable_completions_per_prompt = False  # Whether prompts have different completion counts
        self.requires_request_aware_processing = False  # Whether to use request_id-based grouping
        if self.use_vllm:
    ...
    def _wait_queue(self, timeout=300):
        """
        Wait for queue with timeout to prevent infinite blocking.
        
        Args:
            timeout: Maximum time to wait in seconds (default: 5 minutes)
            
        Returns:
            bool: True if queue has items, False if timeout occurred
        ...
        while self._queue.empty():
            if time.time() - start_time > timeout:
                logger.error(f'Queue wait timeout after {timeout}s - potential deadlock detected')
    ...
    def _check_queue_health(self):
        """
        Check the health of the async generation queue.
        
        Returns:
            dict: Queue health status information
        """
        health_info = {
        ...
        if self.accelerator.is_main_process:
            logger.info(f'Queue health: {health_info}')
    ...
    def _infer_single_or_multi_turn(self,
        ...
        """
        # Prepare inputs with system prompts
        ...
        # Handle single-turn or server multi-turn
    ...
    def async_generate_rollout(self, all_inputs):
        ...
        def infer_task():
            try:
                ...
                # Re-raise to ensure the exception is captured by the future
                raise
        ...
        def done(future):
            try:
                ...
                # Put a sentinel value to prevent deadlock in consumers
                try:
                    current_queue.put(DataCache(None))  # Sentinel to indicate failure
                except Exception as queue_error:
                    logger.error('Failed to put sentinel in queue: %s', str(queue_error))
    ...
    def _fast_infer(self, inputs: DataType) -> DataType:
        """Efficient inference with vLLM colocate mode, async generation, and model offloading."""
        logger.info(f'Process {self.accelerator.process_index}: _fast_infer called with {len(inputs)} inputs')

        # Wake up engine if sleeping
        ...
        # Load model weights if global_step changed
        if self.state.global_step != self._last_loaded_step:
            self._move_model_to_vllm()
            self._last_loaded_step = self.state.global_step

        # Offload model/optimizer if enabled
        context = self.offload_context if self.enable_offload else nullcontext
        with context():
            set_expandable_segments(False)
            
            # Wake up kv_cache after offloading
            ...
            # Handle rollout for async or sync generation
            if self.async_generate:
                all_inputs = gather_object(inputs)
                self.async_generate_rollout(all_inputs)

                try:
                    data_cache: DataCache = self._queue.get(timeout=300)
                    if data_cache.results is None:
                        logger.error('Async generation failed - received sentinel value')
                        raise RuntimeError('Async generation task failed')
                    
                    all_outputs = gather_object(data_cache.results)
                except Exception as e:
                    logger.error(f'Failed to retrieve async generation results: {e}')
                    raise RuntimeError(f'Async generation retrieval failed: {e}') from e
            ...
            # Reset prefix cache and sleep to release memory
            if self.vllm_mode == 'colocate' and self.args.sleep_level > 0:
                self.engine.engine.reset_prefix_cache()
        ...
        logger.info(f'Process {self.accelerator.process_index}: _fast_infer completed, returning {len(outputs)} outputs')
        return outputs

    def _get_even_process_data(self, data: list) -> list:
        """
        Distribute data evenly across processes using contiguous slicing.
        Each process gets its own slice of the data.
        """
        if not data:
            return []
        ...
        # Distribute samples using contiguous slices
        ...
        if self.accelerator.process_index < remainder:
            start_idx = self.accelerator.process_index * (samples_per_process + 1)
            end_idx = start_idx + samples_per_process + 1
        else:
            start_idx = remainder * (samples_per_process + 1) + \
                       (self.accelerator.process_index - remainder) * samples_per_process
            end_idx = start_idx + samples_per_process
        
        local_slice = data[start_idx:end_idx]
        return local_slice
    ...
    def _generate_completions(self, inputs: DataType) -> DataType:
        ...
        if not inputs:
            logger.info(f'Process {self.accelerator.process_index}: '
                       f'_generate_completions received empty inputs, returning early')
            return []
        
        logger.info(f'Process {self.accelerator.process_index}: '
                   f'_generate_completions processing {len(inputs)} inputs')
        ...
        logger.info(f'Process {self.accelerator.process_index}: '
                   f'_generate_completions completed, returning {len(results)} results')
        return results
    ...
    def _generate_and_score_completions(self, inputs: DataType) -> DataType:
        ...
        # ===== SCOUTING PHASE =====
        # Generate completions for all inputs
    ...
    def _score_completions(self, inputs: DataType) -> torch.Tensor:
        ...
        if not inputs:
            ...
            # Return empty tensor with correct shape (0, num_reward_funcs)
            num_reward_funcs = len(self.reward_funcs) if hasattr(self, 'reward_funcs') else 1
            return torch.zeros((0, num_reward_funcs), dtype=torch.float32, device=device)
        ...
        if self.use_gym_env:
            ...
            # Extract hidden states if any reward function needs them
            hidden_states_data = self._extract_hidden_states_for_rewards(inputs)
            
            # Compute rewards using reward functions
            local_rewards_per_func = self._compute_rewards_per_func(inputs, hidden_states_data)

        # gather rewards
        if not self.requires_request_aware_processing:
    ...
    def _extract_hidden_states_for_rewards(self, inputs: DataType) -> Optional[Dict[str, Any]]:
        """
        Extract hidden states from model for reward computation.
        Only performs forward pass if needed by reward functions.
        
        Args:
            inputs: List of input examples with messages and completions
            
        Returns:
            Dictionary containing hidden states and related data, or None if not needed
        """
        # Check if any reward function needs hidden states
        # Keywords that indicate hidden state requirement
        hidden_state_keywords = ['hiddenstate', 'stability', 'representational', 'alignment', 'visual', 'semantic', 'flow', 'combined', 'answer', 'reasoning', 'consistency', 'hierarchical', 'differentiated']
        needs_hidden_states = any(
            any(keyword in reward_func_name.lower().replace('_', '') for keyword in hidden_state_keywords)
            for reward_func_name in self.reward_func_names
        )
        
        if not needs_hidden_states:
            logger.info(f"No reward functions need hidden states. Reward functions: {self.reward_func_names}")
            return None
        
        logger.info(f"Extracting hidden states for {len(inputs)} completions with reward functions: {self.reward_func_names}")
        
        try:
            # Encode the full sequences (prompt + completion)
            template = self.template
            encoded_data = []
            
            # 🔑 CRITICAL: Ensure template is in train mode to generate labels
            original_mode = getattr(template, '_mode', None)
            ...
            with self._template_context(template):
                for idx, data in enumerate(inputs):
                    try:
                        ...
                        logger.error(f"Failed to encode input {idx}: {e}")
                        # Use a fallback empty encoding
                        raise
                
                # Collate into batch
                batch_inputs = to_device(template.data_collator(encoded_data), self.model.device)
            
            # Restore original template mode
            if hasattr(template, 'set_mode') and original_mode is not None:
                template.set_mode(original_mode)
            
            logger.debug(f"Encoded batch - input_ids shape: {batch_inputs['input_ids'].shape}")
            
            # Forward pass to get hidden states
            # Prepare model inputs - handle multimodal models
            model_inputs = {
            ...
            # Add additional inputs for multimodal models if available
            for key in ['pixel_values', 'image_grid_thw', 'images']:
                if key in batch_inputs and batch_inputs[key] is not None:
                    model_inputs[key] = batch_inputs[key]
                    logger.debug(f"Added {key} to model inputs for multimodal model")
            
            with torch.no_grad():
                outputs = self.model(**model_inputs)
            
            # Extract hidden states (tuple of tensors, one per layer)
            all_hidden_states = outputs.hidden_states  # Tuple[Tensor[batch, seq_len, hidden_dim]]
            
            if all_hidden_states is None:
                logger.error("Model did not return hidden states!")
                return None
            ...
            hidden_states_data = {
                ...
                'input_ids': batch_inputs['input_ids'],
                'attention_mask': batch_inputs.get('attention_mask'),
                'labels': batch_inputs.get('labels'),  # 🔑 CRITICAL: Pass labels for completion masking
                'tokenizer': self.processing_class,  # 🔑 CRITICAL: Pass tokenizer for zone parsing
            }
            
            
            logger.debug(f"Extracted hidden states from {len(all_hidden_states)} layers "
                        f"for {len(inputs)} sequences (seq_len: {all_hidden_states[0].shape[1]})")
            return hidden_states_data
            
        except Exception as e:
            logger.error(f"Failed to extract hidden states for reward computation: {e}", exc_info=True)
            logger.error(f"This may happen with complex multimodal inputs. Hidden state rewards will be zero.")
            return None
    
    def _compute_rewards_per_func(self, inputs: DataType, hidden_states_data: Optional[Dict[str, Any]] = None) -> torch.Tensor:
        """Compute rewards using all reward functions
        
        Args:
            inputs: List of input examples
            hidden_states_data: Optional pre-computed hidden states for reward functions that need them
        """
        device = self.accelerator.device
        ...
        for i, (reward_func, reward_model_plugin, reward_func_name) in enumerate(
            ...
            with patch_profiling_context(self, reward_func_name):
                # reward model
                reward_kwargs = {'trainer_state': self.state, 'model': self.model, 'template': self.template}
                
                # Add hidden states if available and this reward function might need them
                hidden_state_keywords = ['hiddenstate', 'stability', 'representational', 'alignment', 'visual', 'semantic', 'flow', 'combined', 'answer', 'reasoning', 'consistency']
                needs_hidden_states = any(keyword in reward_func_name.lower().replace('_', '') for keyword in hidden_state_keywords)
                
                if hidden_states_data is not None and needs_hidden_states:
                    reward_kwargs.update(hidden_states_data)
                    logger.debug(f"Passing hidden states to reward function: {reward_func_name}")
                ...
                # reward function
                    ...
                    # CRITICAL: Preserve hidden_states_data parameters (labels, tokenizer, etc.) 
                    # by updating RowPreprocessor data first, then preserving hidden_states_data
                    row_preprocessor_data = RowPreprocessor.rows_to_batched(inputs)
                    # Also pass through the original per-sample inputs so rewards can access full records
                    reward_kwargs['inputs'] = inputs
                    
                    # Store critical hidden_states_data parameters before they get overwritten
                    protected_params = {}
                    if hidden_states_data is not None:
                        critical_params = ['labels', 'tokenizer', 'all_hidden_states', 'lm_head', 'attention_mask']
                        for param in critical_params:
                            if param in hidden_states_data:
                                protected_params[param] = hidden_states_data[param]
                    
                    # Update with RowPreprocessor data first
                    reward_kwargs.update(row_preprocessor_data)
                    
                    # Restore critical hidden_states_data parameters (this overwrites any conflicts)
                    reward_kwargs.update(protected_params)
    ...
    def _compute_advantages(self, inputs: DataType, rewards_per_func: torch.Tensor,
        ...
        # --------------------------------------------------
        # Case 1: Default grouped mode (fixed completions per prompt)
        # --------------------------------------------------
        if not self.requires_request_aware_processing:
        ...
        # --------------------------------------------------
        # Case 2: Request-aware mode (variable completions per prompt)
        # --------------------------------------------------
        else:
            ...
            # Validate rewards consistency within the same request_id
            ...
            # Group rewards by prompt_id and compute prompt-level mean/std
    ...
    def _prepare_batch_inputs(self, inputs: DataType) -> List[DataType]:
        ...
        for batch in gas_chunks:
            ...
            with self._template_context(template):
                ...
                # Store origin data for multimodal processing and Visual-Desc alignment
                if self.requires_request_aware_processing and self.is_multimodal:
                    batch_encoded_inputs['_origin_data'] = batch
                elif self.args.enable_sagpo:
                    batch_encoded_inputs['_origin_data'] = batch
                
                # Cache encoded data for multimodal inputs to avoid re-encoding in chunked processing
                if self.is_multimodal:
                    # Store the encoded data before labels are popped
                    batch_encoded_inputs['_encoded_data_cache'] = [template.encode(data) for data in batch]
            ...
            # Record prompt length for visual path (crucial for answer alignment)
            # Note: labels shape is [batch_size, seq_len], where -100 indicates non-answer tokens
            answer_lengths = (labels != -100).sum(dim=1)  # [batch_size]
            total_lengths = labels.shape[1]  # scalar
            prompt_lengths_visual = total_lengths - answer_lengths  # [batch_size]
            ...
            batch_encoded_inputs.update({
                ...
                'advantages':
                all_advandages,
                'prompt_length': prompt_lengths_visual  # Add prompt length for visual path
            })
            ...
            with torch.no_grad():
                ...
                # Skip reference model computation for Visual-Desc alignment (unless ablation switch is on)
                skip_ref_model = self.args.enable_sagpo and not self.args.sagpo_use_kl_penalty
                if self.beta == 0.0 or skip_ref_model:
    ...
    def _compute_loss(self, model, inputs):
        ...
        should_chunk = self.has_variable_rollout_counts and any(gather_object([batch_size > expected_bs]))
        if not should_chunk:
    ...
    def _compute_loss_and_metrics(self, model, inputs):
        """Core loss computation with hidden state alignment."""
        mode = 'train' if self.model.training else 'eval'

        completion_mask = inputs['completion_mask']
        truncated_mask = inputs['truncated_mask']

        # Execute GRPO forward pass
        if self.accelerator.is_main_process:
            if self.args.enable_sagpo and mode == 'train':
                kl_status = "enabled" if self.args.sagpo_use_kl_penalty else "disabled"
                logger.info(f"Visual-Desc alignment enabled (α={self.args.sagpo_alpha}, KL penalty: {kl_status})")
        
        # Execute clean forward pass and get logps, entropies, and logits
        per_token_logps, entropies, student_logits_clean = self._get_logps_entropies_and_logits(
            model, inputs, compute_entropy=self.compute_entropy)
        
        # Compute GRPO loss
        # Initialize alignment_loss to None - will be computed separately if needed
        alignment_loss = None
        ...
        # Skip KL constraint for Visual-Desc alignment (unless ablation switch is on)
        skip_kl_penalty = self.args.enable_sagpo and not self.args.sagpo_use_kl_penalty
        if self.beta != 0.0 and not skip_kl_penalty and not self.kl_in_reward:
        ...
        if per_token_kl is not None:        
            # Compute GRPO loss with KL penalty (if beta != 0 and not skipping KL for Visual-Desc alignment)
            if self.beta != 0.0 and not skip_kl_penalty:
        ...
        # Compute Visual-Desc alignment loss
        if self.args.enable_sagpo and mode == 'train':
            try:
                # Check if we have origin data with descriptions
                if '_origin_data' not in inputs or not inputs['_origin_data']:
                    raise ValueError("Visual-Desc alignment requires '_origin_data' to be present in inputs")
                
                caption_field = self.args.sagpo_caption_field
                has_caption = any(
                    caption_field in item and item[caption_field] 
                    for item in inputs['_origin_data']
                )
                
                if not has_caption:
                    raise ValueError(f"Visual-Desc alignment caption field '{caption_field}' not found in any sample")
                
                # Prepare desc inputs
                desc_inputs = self._prepare_desc_inputs(inputs, caption_field)
                
                if desc_inputs is None:
                    raise RuntimeError("Failed to prepare desc inputs")
                
                # Memory check before desc processing (for hidden state method)
                self._check_memory_before_forward(desc_inputs, "desc")
                
                # Compute alignment loss using hidden state alignment (NEW METHOD)
                # Get tokenizer for encoding special tags
                tokenizer = getattr(self.model, 'tokenizer', None)
                if tokenizer is None:
                    # Try to get tokenizer from template or other sources
                    if hasattr(self, 'template') and hasattr(self.template, 'tokenizer'):
                        tokenizer = self.template.tokenizer
                    else:
                        raise RuntimeError("Cannot find tokenizer for hidden state alignment loss")
                
                # CRITICAL FIX: Ensure alignment loss participates in gradient computation
                # The desc path must also compute gradients for the alignment loss
                alignment_loss = self._compute_hidden_state_based_alignment_loss(
                    inputs, desc_inputs, tokenizer)
                
                # Clean up memory
                self._memory_cleanup_and_monitor()
                
                if self.accelerator.is_main_process:
                    logger.info(f"Alignment loss: {alignment_loss.item():.6f}")
                
            except Exception as e:
                logger.error(f"Failed to compute Visual-Desc alignment loss: {e}", exc_info=True)
                alignment_loss = None
        
        # Combine final loss
        if alignment_loss is not None:
            # CRITICAL FIX: Ensure alignment_loss has gradients
            if not alignment_loss.requires_grad:
                logger.warning("Alignment loss does not require gradients! This will break Visual-Desc alignment training.")
                # Force gradient computation by adding a small perturbation
                alignment_loss = alignment_loss + 0.0 * torch.sum(torch.tensor(1.0, device=alignment_loss.device, requires_grad=True))
            
            sagpo_loss = self.args.sagpo_alpha * alignment_loss
            loss = grpo_loss + sagpo_loss
            
            if self.accelerator.is_main_process:
                logger.info(f"Visual-Desc alignment loss: {loss.item():.6f} (GRPO: {grpo_loss.item():.6f}, Alignment: {alignment_loss.item():.6f})")
                logger.info(f"Gradient check - GRPO loss requires_grad: {grpo_loss.requires_grad}, Alignment loss requires_grad: {alignment_loss.requires_grad}")
                logger.info(f"Total loss requires_grad: {loss.requires_grad}")
        else:
            loss = grpo_loss
            if self.accelerator.is_main_process:
                logger.info(f"GRPO loss: {loss.item():.6f}")
        ...
        # Add Visual-Desc alignment metrics if available
        if alignment_loss is not None:
            metrics_data['sagpo_hidden_state_alignment_loss'] = alignment_loss.item()
            metrics_data['sagpo_total_loss'] = loss.item()
            metrics_data['sagpo_alpha'] = self.args.sagpo_alpha
            metrics_data['sagpo_contribution'] = (self.args.sagpo_alpha * alignment_loss.item())
            
            # Calculate Visual-Desc alignment ratio with zero division protection
            grpo_loss_abs = abs(grpo_loss.item())
            if grpo_loss_abs > 1e-8:  # Avoid division by zero
                metrics_data['sagpo_ratio'] = (self.args.sagpo_alpha * alignment_loss.item()) / grpo_loss_abs * 100
            else:
                metrics_data['sagpo_ratio'] = 0.0  # Set to 0 when GRPO loss is too small

        if self.beta != 0.0 and not skip_kl_penalty and per_token_kl is not None:
    ...
    def _update_metrics(self, metrics_data):
        ...
        # Update Visual-Desc alignment metrics
        if 'sagpo_hidden_state_alignment_loss' in metrics_data:
            self._metrics[mode]['sagpo/hidden_state_alignment_loss'].append(metrics_data['sagpo_hidden_state_alignment_loss'])
            self._metrics[mode]['sagpo/alpha'].append(metrics_data['sagpo_alpha'])
            self._metrics[mode]['sagpo/contribution'].append(metrics_data['sagpo_contribution'])
            self._metrics[mode]['sagpo/ratio'].append(metrics_data['sagpo_ratio'])
    ...
    def _get_logps_entropies_and_logits(self,
                                       model,
                                       inputs,
                                       compute_entropy=False) -> Tuple[torch.Tensor, Optional[torch.Tensor], torch.Tensor]:
        """
        Compute per-token log probabilities, entropies, and logits simultaneously.
        This is needed for the decoupled Visual-Desc alignment implementation where we need to
        preserve clean logits for alignment loss computation.
        
        Returns:
            Tuple of (per_token_logps, entropies, logits)
        """
        batch_size = inputs['input_ids'].shape[0]
        mode = 'train' if self.model.training else 'eval'
        expected_bs = self.args.per_device_train_batch_size if mode == 'train' else self.args.per_device_eval_batch_size
        ...
        if not should_chunk:
            return self._get_logps_entropies_and_logits_single(model, inputs, compute_entropy=compute_entropy)
        else:
            # For chunked processing, we need to handle logits differently
            # For now, fall back to the standard approach and extract logits separately
            logps, entropies = self._get_per_token_logps_and_entropies_chunked(model, inputs, compute_entropy=compute_entropy)
            # We'll need to do a separate forward pass to get logits for chunked case
            # This is a limitation but should be acceptable for most use cases
            with torch.no_grad():
                inputs_for_logits = {
                ...
                if 'logits_to_keep' in self.model_kwarg_keys:
                    inputs_for_logits['logits_to_keep'] = inputs['logits_to_keep'] + 1
                logits = model(**inputs_for_logits).logits
                logits = logits[:, -(inputs['logits_to_keep'] + 1):-1, :]
            return logps, entropies, logits

    def _get_logps_entropies_and_logits_single(self,
                                              model,
                                              inputs,
                                              compute_entropy=False) -> Tuple[torch.Tensor, Optional[torch.Tensor], torch.Tensor]:
        """
        Compute per-token log probabilities, entropies, and logits for a single batch.
        """
        logits_to_keep = inputs['logits_to_keep']
        input_ids = inputs['input_ids']
        unwrapped_model = self.accelerator.unwrap_model(model)
        ...
        if can_use_super:
            # Use the parent implementation for logps and entropies
            if hasattr(super(), '_get_per_token_logps_and_entropies'):
                logps, entropies = super()._get_per_token_logps_and_entropies(
                    model, input_ids, inputs['attention_mask'], logits_to_keep, compute_entropy=compute_entropy)
            else:
            ...
            # We still need to get the logits separately for this case
            inputs_for_logits = {
                k: v for k, v in inputs.items() if k not in [
                    'logits_to_keep', 'completion_mask', 'ref_per_token_logps', 'advantages', 'old_per_token_logps',
                    'truncated_mask'
            ...
            if 'logits_to_keep' in self.model_kwarg_keys:
                inputs_for_logits['logits_to_keep'] = logits_to_keep + 1
            logits = model(**inputs_for_logits).logits
            logits = logits[:, -(logits_to_keep + 1):-1, :]
        else:
            # Standard path: get everything together
            inputs_clean = {
                k: v for k, v in inputs.items() if k not in [
                    'logits_to_keep', 'completion_mask', 'ref_per_token_logps', 'advantages', 'old_per_token_logps',
                    'truncated_mask'
            ...
            if 'logits_to_keep' in self.model_kwarg_keys:
                inputs_clean['logits_to_keep'] = logits_to_keep + 1
            
            logits = model(**inputs_clean).logits
            # exclude the last logit: it corresponds to the next token pred
            logits = logits[:, -(logits_to_keep + 1):-1, :]
            logits = logits / self.temperature
            input_ids_aligned = input_ids[:, -logits_to_keep:]
            logps = selective_log_softmax(logits, input_ids_aligned)
    ...
    def _server_rollout(self, inputs: DataType, request_config: RequestConfig,
        ...
        if self.enable_server_multi_turn:
            ...
            if outputs_count != len(all_requests):
                ...
                self.has_variable_rollout_counts = True
                self.requires_request_aware_processing = True
                if self.args.dynamic_sample:
        ...
        if not is_global_inputs:
            ...
            if not self.enable_server_multi_turn or not self.has_variable_rollout_counts:
    ...
    def _colocate_rollout(self, inputs: DataType, request_config: RequestConfig) -> List[RolloutOutput]:
        ...
        if self.vllm_tensor_parallel_size > 1:
            ...
            # Gather input lengths from all ranks in the TP group
            ...
            # Gather actual inputs from all TP group ranks
    ...
    def _postprocess_rollout_outputs(self, inputs: DataType, outputs: List[RolloutOutput]) -> DataType:
        ...
        def merge_output_input_data(input_data: Dict[str, Union[torch.Tensor, Any]], output: RolloutOutput):
            ...
            # Override multi-modal data from rollout_infos
            if output.rollout_infos:
